...
function InterviewPageInner() {
  ...
  async function handleSkip() {
    ...
    setError(null);
    try {
      await api.skipInterview(Number(datasetId));
      router.push(`/kpi-selection/${datasetId}`);
    } catch (e) {
      setError(String(e));
      setSkipping(false);
    }
  ...
  if (!uploadId && !datasetId) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        <p className="text-red-600">Missing upload or dataset ID. Go back and upload a file.</p>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-2xl px-4 py-10 flex flex-col gap-6">
      <div className="flex items-start justify-between">
        ...
        {datasetId && !done && (
          <button
            ...
            title="Skip the interview — AI will build the recipe from your schema and data model"
          >
            {skipping ? "Skipping…" : "Skip interview →"}
          </button>
        )}
      </div>
      ...
      <ol className="flex gap-1">
        {STEP_LABELS.map((label, i) => {
          ...
          const isDone = idx < stepIndex || done;
          return (
      ...
      {done ? (
        <div className="flex items-center gap-3">
          <p className="text-sm text-green-700 flex-1">All done! Ready to generate your report recipe.</p>
          <button
