from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware

from core.config import settings
from api.routes import upload as upload_router
from api.routes import interview as interview_router
from api.routes import kpis as kpis_router
from api.routes import reports as reports_router
from api.routes import analysis as analysis_router
from api.routes import export as export_router
from api.routes import registry as registry_router
from db.database import init_db
from core.database import engine, Base
import models  # noqa: F401 — registers all ORM models with Base

app = FastAPI(
    title="BGO Executive Dashboard",
    version="2.0.0",
    docs_url="/docs" if settings.app_env != "production" else None,
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://localhost:3000", "*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Create SQLAlchemy ORM tables (datasets, uploads, staging_tables, etc.)
Base.metadata.create_all(bind=engine)
# Ensure SQLite schema tables exist on startup (kpi_catalog, schema_memory, etc.)
init_db()

app.include_router(upload_router.router)
app.include_router(interview_router.router)
app.include_router(kpis_router.router)
app.include_router(reports_router.router)
app.include_router(analysis_router.router)
app.include_router(export_router.router)
app.include_router(registry_router.router)


@app.get("/health")
def health():
    return {"status": "ok", "app": "BGO Executive Dashboard", "version": "2.0.0"}
