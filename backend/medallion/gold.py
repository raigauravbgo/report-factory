...
def _apply_kpi(kpi_def: dict, frames: dict[str, dict]) -> dict:
    ...
    required_cols = kpi_def.get("required_columns", []) or kpi_def.get("source_fields", [])
    ...
    if required_cols and not set(required_cols).issubset(available_cols):
        return {"value": None, "breakdown": None}

    # ── RF-master numerator/denominator model (for KPIs without a formula) ──
    if not formula:
        numerator = kpi_def.get("numerator")
        denominator = kpi_def.get("denominator")
        if numerator and denominator and numerator in available_cols and denominator in available_cols:
            denom_sum = combined[denominator].sum()
            rate = (combined[numerator].sum() / denom_sum * 100) if denom_sum else 0
