...
| AI / Agent | Google ADK (local) + Claude via Anthropic API + OpenAI/Azure OpenAI |
| Charts | Recharts 3.x |
...
│   │                              #   referential integrity check (samples FK values)
...
├── catalog/
│   ├── kpis.json                  # 58 BGO KPIs (collections, cx, sales, workforce) — do not alter structure
...
│   ├── dashboard/[recipeId]/page.tsx             # Metrics, trend charts, driver bars, benchmark insights
...
| `kpi_catalog` | 58 BGO KPIs (seeded from `catalog/kpis.json`) |
...
Insights are **rule-based templates** — no LLM call on dashboard load. Two shapes:
...
| File | Why it matters |
|---|---|
| `backend/catalog/kpis.json` | 58 BGO KPIs — source of truth for all KPI suggestions |
